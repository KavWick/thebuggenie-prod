<?php
	...
	const COMMENT = 'issue_spenttimes.comment';
        ...
        public function getSpentTimeByDateAndUserID($startdate, $enddate, $user_id)
        {
            $crit = $this->getCriteria();
            $crit->addWhere(self::EDITED_BY, $user_id);
            $crit->addOrderBy(self::EDITED_AT, Criteria::SORT_DESC);
            ...
            if ($res)
            {
                while ($row = $res->getNextRow())
                {
                    $ret_arr[$row[self::ID]]['spenttime'] = IssueSpentTimes::getTable()->selectById($row->get(self::ID));
                    $ret_arr[$row[self::ID]]['issue'] = Issues::getTable()->selectById($row->get(self::ISSUE_ID));
